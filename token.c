	...
	while((ch=fgetc(file->f_in)) != '\"')
	{
		switch(ch)
		{
			...
			case '%' :fputs("<span class=\"character\">",file->f_out);
				  fputc(ch,file->f_out);
				  ch=fgetc(file->f_in);
				 while (ch != 'd' && ch != 'i' && ch != 'u' && ch != 'o' && ch != 'x' && ch != 'X' && ch != 'f' && ch != 'F' && ch != 'e' && ch != 'E' &&
       					ch != 'g' && ch != 'G' && ch != 'a' && ch != 'A' && ch != 'c' && ch != 's' && ch != 'p' && ch != 'n' && ch != '%' && ch!='.' && ( ch<'0' && ch>'					9')) 
				{
				  	fputc(ch,file->f_out);
					ch=fgetc(file->f_in);
				}
...
void write_direcive(int ch,struct files *file)
{
